#! usr/bin/python
# This program is used to return the maximum clique present in the given graph. This is a modifincation of the program depth_first_prune2.py with addition of the coloring. Basically we are adding a condition depending upon the color of the neightbourhood 
...
COVR = [] # This is for collecting the vertex which will be covered by others
MAX_CLIQUE = []# This is to store max clique
...
for i in range (0,len(V)):
      ...
      for i1 in range(0,len(COVR)):
            if V[i]==COVR[i1]:
                  flag = 1 #
      # For chaking the color condition
      col_max = c.Coloring(T_p,Adj_mat)
      if len(Q)+col_max <= len(MAX_CLIQUE):
          flag = 1      
